...
class Application:
    ...
    def estimate_tx_fees(self, web3, blockchain, transaction, address_from):
        ...
        return {'estimated_gas': estimated_gas, 'estimated_fees': estimated_fees, 'price': price}
            ...
            for transaction in response["result"]:
                if transaction["from"].lower() == address.lower() and transaction["isError"] == '0':
                    ...
                    playbook[current_blockchain].append(
                        {'hash': transaction['hash'], 'timestamp': transaction["timeStamp"]}
                    )
            print(
                '{} Written {} transactions on {} in {}'.format(
                    colored('[INFO]', 'magenta'), nb_tx, current_blockchain, playbook_filename
                )
            )
    ...
    def load_transactions(self, playbook):
        ...
        for blockchain in parsed_playbook:
            ...
            for transaction_hash in parsed_playbook[blockchain]:
                self.transactions[blockchain][transaction_hash] = web3.eth.get_transaction(
                    transaction_hash
                )
    ...
    def replicate(self, password, playbook, blockchains):
        ...
        for blockchain in blockchains:
            ...
            for tx_hash, tx in self.transactions[blockchain].items():
                ...
                if not confirm:
                    print(
                        '{} Skipped transaction {}.'.format(colored('[INFO]', 'magenta'), tx_hash)
                    )
                else:
