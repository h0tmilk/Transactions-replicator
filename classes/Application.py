...
class Application:
    ...
    def create_accounts(self, number, path, password):
        for x in range(number):
            ...
            keyfile_content = eth_keyfile.create_keyfile_json(
    ...
    def load_accounts(self, password):
        for keyfile in os.listdir(self.accounts_path):
            ...
            try:
                self.accounts[
                    "0x" + keyfile_json["address"].lower()
                ] = eth_keyfile.decode_keyfile_json(
                    keyfile_json, password.encode()
                ).hex()
            except ValueError:
                raise ValueError(
                    "{} Incorrect password.".format(colored("[ERROR]", "red"))
                )
    ...
    def get_gas_oracle(self, blockchain):
        ...
        query = {"module": "gastracker", "action": "gasoracle", "apikey": api_key}
        headers = {
    ...
    def estimate_tx_fees(self, web3, blockchain, transaction, address_from):
        ...
        estimated_fees = round(
            estimated_gas * float(web3.fromWei(web3.toWei(price, "gwei"), "ether")), 5
        )
    ...
    def extract_transactions_from_address(self, address, blockchains):
        ...
        for current_blockchain in blockchains:
            ...
            response = json.loads(
                requests.get(url_api, headers=headers, params=query).text
            )
            ...
            for transaction in response["result"]:
                if (
                    transaction["from"].lower() == address.lower()
                    and transaction["isError"] == "0"
                ):
                    nb_tx += 1
                    playbook[current_blockchain].append(
                        {
            ...
            print(
                "{} Written {} transactions on {} in {}".format(
            ...
            playbook[current_blockchain] = [
                d["hash"] for d in playbook[current_blockchain]
            ]
    ...
    def replicate(self, password, playbook, blockchains):
        ...
        for blockchain in blockchains:
            ...
            for tx_hash, tx in self.transactions[blockchain].items():
                # Estimate total fees
                test_address = (
                    list(self.accounts.keys())[0]
                    if web3.toChecksumAddress(list(self.accounts.keys())[0])
                    != tx["from"]
                ...
                for address, private_key in self.accounts.items():
                    if web3.toChecksumAddress(address) != web3.toChecksumAddress(
                        tx["from"]
                    ):
                ...
                confirm = confirmation(
                    "{} Replicate transaction {} on {} accounts for {} {} fees ({} {} each)".format(
                ...
                if not confirm:
                    print(
                        "{} Skipped transaction {}.".format(
                            colored("[INFO]", "magenta"), tx_hash
                        )
                    )
                else:
    ...
    def dispatch_currency(self, amount, from_address, blockchain, password):
        ...
        if from_address.lower() not in self.accounts:
            raise ValueError(
                "{} {} is not in {}.".format(
                    colored("[ERROR]", "red"), from_address, self.accounts_path
                )
            )
        ...
        confirm = confirmation(
            "{} {} {} will be sent to {} addresse(s)".format(
                colored("[CONFIRM]", "blue"),
                str(total_amount),
        ...
        if balance <= amount:
            raise ValueError(
                "{} Address {} has only {} {} and the operation needs at least {} {}".format(
                    colored("[ERROR]", "red"),
                    from_address,
                    str(balance),
                    self.conf.blockchains[blockchain]["symbol"],
        ...
        confirm = confirmation(
            "{} The estimated total gas fees for the operation(s) is estimated at {} {}".format(
                colored("[CONFIRM]", "blue"),
                str(estimated_total_fees),
                str(self.conf.blockchains[blockchain]["symbol"]),
            ),
            "no",
        )
            ...
            if address != from_address.lower():
                ...
                print(
                    "{} {} ----- {} {} -----> {} : {}tx/{}".format(
                        ...
                        str(self.conf.blockchains[blockchain]["symbol"]),
                        address[:8],
                        self.conf.blockchains[blockchain]["api_url"].replace(
                            "api-", ""
                        ),
                        str(tx_hash.hex()),
                    )
    ...
    def replicate_transaction(self, web3, blockchain, tx, address):
        ...
        print(
            "{} Replicated transaction {} on {}: {}{}".format(
                colored("[TX]", "green"),
                str(tx["hash"].hex()),
                address[:8],
